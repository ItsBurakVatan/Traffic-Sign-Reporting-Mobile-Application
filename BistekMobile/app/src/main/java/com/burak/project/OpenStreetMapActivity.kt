            ...
            .addListenerForSingleValueEvent(object : ValueEventListener {
                override fun onDataChange(dataSnapshot: DataSnapshot) {
                    for (veriSnapshot in dataSnapshot.children) {
                        val aktif = veriSnapshot.child("aktif").getValue(Boolean::class.java)
                        // "id" alanını kontrol et
                        if (aktif == false && veriSnapshot.hasChild("id")){
                            ...
                            if (konum != null) {
                                ...
                                if (latLngArray.size == 4) {
                                    ...
                                    if (latitude != null && longitude != null) {
                                        ...
                                        val intent = intent
                                        val id = veriSnapshot.child("id").getValue(String::class.java)
                                        val direk = veriSnapshot.child("direk").getValue(String::class.java)
                                        val levha = veriSnapshot.child("levha").getValue(String::class.java)

                                        // Marker'ı oluştur ve bilgileri ekleyerek haritaya ekle
                                        val marker = Marker(mapView)
                                        marker.position = geoPoint
                                        marker.snippet = "ID: $id, Direk: $direk, Levha: $levha"
